...
bool isCompilingForAndroid(const llvm::Module &m) {
  // android target sample: aarch64-none-linux-android
  llvm::Regex reAndroid(".*android.*");
  std::string target(m.getTargetTriple());
  return reAndroid.match(target) ? true : false;
}

bool isCompilingForIOS(const llvm::Module &m) {
  // iOS target sample: arm64-apple-ios12.4
  llvm::Regex reAndroid(".*ios.*");
  std::string target(m.getTargetTriple());
  return reAndroid.match(target) ? true : false;
}
